...
async def admin_users(call: CallbackQuery):
    ...
    msg = f"👥 <b>Foydalanuvchilar ({len(users)} ta):</b>\n\n"
    for u in users[:30]:  # max 30 ta ko'rsatamiz
        ...
        org = u.get('organization') or u.get('faculty') or '—'

        msg += f"{role_e} <b>{u['full_name']}</b>{admin_b}\n"
        msg += f"   ID: <code>{u['telegram_id']}</code>\n"
...
async def main():
    ...
    # Eski webhook/polling sessiyalarini tozalaymiz
