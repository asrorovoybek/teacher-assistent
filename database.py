...
def get_user(telegram_id: int):
    res = safe_execute(supabase.table("users").select("*").eq("telegram_id", telegram_id), default=None)
...
def create_user(telegram_id: int, full_name: str, username: str = None):
    ...
    safe_execute(supabase.table("users").insert({
    ...
    safe_execute(supabase.table("user_settings").insert({"user_id": telegram_id}))
    return get_user(telegram_id)


def update_user(telegram_id: int, data: dict):
    safe_execute(supabase.table("users").update(data).eq("telegram_id", telegram_id))
...
def is_admin(telegram_id: int) -> bool:
    res = safe_execute(supabase.table("users").select("is_admin").eq("telegram_id", telegram_id), default=None)
...
def get_all_users():
    res = safe_execute(supabase.table("users").select("*"), default=None)
    return res.data if res and getattr(res, "data", None) else []


def set_admin(telegram_id: int, status: bool):
    safe_execute(supabase.table("users").update({"is_admin": status}).eq("telegram_id", telegram_id))
...
def add_schedule(user_id: int, day: int, subject: str, room: str, group_name: str,
    ...
    safe_execute(supabase.table("schedule").insert({
        "user_id": user_id,
        "day_of_week": day,
        "subject": subject,
        "room": room,
...
def delete_schedule(schedule_id: int):
    safe_execute(supabase.table("schedule").delete().eq("id", schedule_id))
...
def add_task(user_id: int, title: str, description: str = None, due_date: str = None):
    safe_execute(supabase.table("tasks").insert({
        "user_id": user_id,
        "title": title,
...
def complete_task(task_id: int):
    safe_execute(supabase.table("tasks").update({"is_done": True}).eq("id", task_id))
...
def add_reminder(user_id: int, title: str, remind_at: str, repeat_type: str = "none"):
    safe_execute(supabase.table("reminders").insert({
        "user_id": user_id,
        "title": title,
        "remind_at": remind_at,
...
def get_pending_reminders():
    ...
    res = safe_execute(
        supabase.table("reminders").select("*").eq("is_sent", False).lte("remind_at", now),
        default=None,
    )
    return res.data if res and getattr(res, "data", None) else []
...
def mark_reminder_sent(reminder_id: int):
    safe_execute(supabase.table("reminders").update({"is_sent": True}).eq("id", reminder_id))


def get_user_reminders(user_id: int):
    res = safe_execute(
        supabase.table("reminders").select("*").eq("user_id", user_id).eq("is_sent", False).order("remind_at"),
        default=None,
    )
    return res.data if res and getattr(res, "data", None) else []
...
def get_settings(user_id: int):
    res = safe_execute(supabase.table("user_settings").select("*").eq("user_id", user_id), default=None)
    return res.data[0] if res and getattr(res, "data", None) else None


def update_settings(user_id: int, data: dict):
    safe_execute(supabase.table("user_settings").update(data).eq("user_id", user_id))
...
def get_stats():
    users = safe_execute(supabase.table("users").select("telegram_id", count="exact"), default=None)
    tasks = safe_execute(supabase.table("tasks").select("id", count="exact"), default=None)
    schedules = safe_execute(supabase.table("schedule").select("id", count="exact"), default=None)
    reminders = safe_execute(supabase.table("reminders").select("id", count="exact"), default=None)
    return {
